...
def get_random_indices(num_rows, num_cols, corruption_level=4):
    ...
    if num_cols > 1:
        cont_idxs = zip(
            random.choices(range(num_rows), k=contamination_fraction),
...
def duplicate_rows(data, corruption_level=4):
    ...
    """
    n_rows, _ = data.shape
    n_rows_duplicated = int(np.ceil(n_rows * (0.6 * corruption_level / 10)))
...
def duplicate_columns(data, corruption_level=4):
    ...
    """
    _, n_cols = data.shape
    n_cols_duplicated = int(np.ceil(n_cols * (0.6 * corruption_level / 10)))
